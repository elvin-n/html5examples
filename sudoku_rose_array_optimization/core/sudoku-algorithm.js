...
ArrayStorage.prototype.getNewArray = function ( size ){
  if( typeof this.index[size] === 'undefined' ) {
    // allocate new pull and initialize it
    var s = new Array(this.growSize);
    for ( var i=0; i< this.growSize; i++ ) {
      s[i] = new Array(size);
    }
    this.mainReference[size] = s;
    this.index[size] = { length: this.growSize, head: 0 };
  }
  var cidx = this.index[size];
  if( cidx.head == cidx.length ) {
    var s2 = new Array(this.growSize);
    for ( var i=0; i< this.growSize; i++ ) {
      s2[i] = new Array(size);
    }
    this.mainReference[size].push.apply(this.mainReference[size], s2);
    cidx.length += this.growSize;
  }
  if( cidx.head < cidx.length ) {
    cidx.head++;
    return this.mainReference[size][cidx.head-1];
  }
  else {
    return null;
  }
...
var arrayPool = new ArrayStorage();
...
Field.prototype.step = function()
{
    ...
    var set = arrayPool.getNewArray(refData.length);
  ...
  arrayPool.reset(refData.length);
